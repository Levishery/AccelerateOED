        ...
        for indexMethod in range(len(listMethods)):
            ...
            if listMethods[indexMethod] == 'RANDOM':
                ...
                context = mp.spawn(getMPSequence, nprocs=1, join=False, args=(q, criticalK, isSynchronized,
                                                            ...
                                                            optimalExperiments, update_bond, iterative))
                # sleep(10)
                # q.join()
                context.join()
                [optimalExperiments, update_bond] = q.get()

                MOCUCurve, experimentSequence, timeComplexity = findMPSequence(optimalExperiments, update_bond, MOCUInitial, K_max, w, N, deltaT, MVirtual, MReal, TVirtual, TReal, aLowerUpdated, aUpperUpdated, it_idx, update_cnt)
